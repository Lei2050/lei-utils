...
func NewWorkPool(maxWorkers int) *WorkerPool {
...
func (p *WorkerPool) processWaitingQueue() bool {
	...
	case p.workerQueue <- p.waitingQueue.Front().Value.(func()):
		...
		p.waitingQueue.Remove(p.waitingQueue.Front())
	}
	atomic.StoreInt32(&p.waiting, int32(p.waitingQueue.Len()))
	return true
}
...
func (p *WorkerPool) runQueuedTasks() {
	for p.waitingQueue.Len() != 0 {
		...
		p.workerQueue <- p.waitingQueue.Remove(p.waitingQueue.Front()).(func())
		atomic.StoreInt32(&p.waiting, int32(p.waitingQueue.Len()))
	}
}
